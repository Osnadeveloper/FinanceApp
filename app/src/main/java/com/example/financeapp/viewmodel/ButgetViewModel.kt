...
import com.example.financeapp.data.model.model.Budget
    ...
    fun loadBudgets(userId: Long, month: Int, year: Int) {
        viewModelScope.launch {
            budgetDao.getBudgetsByMonthYear(userId, month, year).collect { budgetList ->
                ...
                updateBudgetStatuses(userId, budgets = budgetList, month, year)
            }
        }
    }

    private suspend fun updateBudgetStatuses(
        userId: Long,
        budgets: List<Budget>,
        month: Int,
        year: Int
        ...
        suspend fun updateBudgetStatuses(
            userId: Long,
            budgets: List<Budget>,
            month: Int,
            year: Int
            ...
            val calendar = Calendar.getInstance()
            // Ajuste de fecha para el rango mensual
            calendar.set(year, month - 1, 1, 0, 0, 0)
            val startDate = calendar.timeInMillis
            ...
            for (budget in budgets) {
                // Obtener el total de gastos para esta categoría y rango de tiempo
                val spent = expenseDao.getTotalExpensesByCategoryAndDateRange(
                    userId,
                    budget.category,
                    startDate,
                ...
                statusList.add(
                    BudgetStatus(
                        ...
                        spent = spent,
                        remaining = budget.monthlyLimit - spent,
                        percentage = if (budget.monthlyLimit > 0)
            ...
            // Actualización del flujo de estado para la UI
